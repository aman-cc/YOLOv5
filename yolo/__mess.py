...
ckpt = torch.load(
    "/data/nextcloud/dbc2017/files/jupyter/data/ckpt/converted_yolov5s.pt"
)
msd = ckpt["model"]
...
torch.save(
    {"ema": (total_new, 0)},
    "/data/nextcloud/dbc2017/files/jupyter/data/ckpt/total_new.pth",
)
...
def parse_dataset(ds, img_sizes):
    ...
    boxes = torch.cat(
        [ds.get_target(id_)["boxes"] * scale for id_, scale in zip(ds.ids, scales)]
    )
