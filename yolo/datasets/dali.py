...
#         self.init_lengths = [
#             shard if i < world_size - extra else shard + 1 for i in range(world_size)
#         ]
...
#         self.pipe = COCOPipeline(
#             file_root, ann_file, batch_size, shuffle, num_threads, device_id, world_size
#         )
...
#                 if not self.shuffle:
#                     self.epoch += 1
#                     self.length = self.init_lengths[
#                         (self.epoch + self.device_id) % self.world_size
#                     ]
