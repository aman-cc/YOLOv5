...
class COCODataset(GeneralizedDataset):
    def __init__(
        self, file_root, ann_file, train=False, transforms=None, index_list=[-1]
    ):
        ...
        self.ids = (
            tuple(str(k) for k in self.coco.imgs) if index_list == [-1] else index_list
        )
        ...
        self.classes = tuple(
            self.coco.cats[k]["name"] for k in sorted(self.coco.cats)
        )  # dense classes
        ...
        self._aspect_ratios = [
            v["width"] / v["height"] for v in self.coco.imgs.values()
        ]
