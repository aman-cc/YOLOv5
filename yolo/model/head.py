...
class Head(nn.Module):
    ...
    def forward(
        self, features, targets, image_shapes=None, scale_factors=None, max_size=None
    ):
    ...
    def compute_loss(self, preds, targets):
        dtype = preds[0].dtype
        image_ids = torch.cat(
            [torch.full_like(tgt["labels"], i) for i, tgt in enumerate(targets)]
        )
        ...
        for pred, stride, wh in zip(preds, self.strides, self.anchors):
            anchor_id, gt_id = box_ops.size_matched_idx(
                wh, gt_boxes[:, 2:], self.match_thresh
            )

            gt_object = torch.zeros_like(pred[..., 4])
            if len(anchor_id) > 0:
                gt_box_xy = gt_boxes[:, :2][gt_id]
                ids, grid_xy = box_ops.assign_targets_to_proposals(
                    gt_box_xy / stride, pred.shape[1:3]
                )
                ...
                gt_object[
                    image_id, grid_xy[:, 1], grid_xy[:, 0], anchor_id
                ...
                losses["loss_cls"] += F.binary_cross_entropy_with_logits(
                    pred_level[..., 5:], gt_label
                )
            losses["loss_obj"] += F.binary_cross_entropy_with_logits(
                pred[..., 4], gt_object
            )
        ...
        for i, im_s in enumerate(image_shapes):  # 20.97s
            ...
            if len(box) > 0:
                ...
                keep = box_ops.batched_nms(
                    box, score, label, self.nms_thresh, max_size
                )  # 4.43s
                ...
                if self.merge:  # slightly increase AP, decrease speed ~14%
                    ...
                    iou = (
                        box_ops.box_iou(nms_box, box) * mask
                    ) > self.nms_thresh  # 1.84s
                    weights = iou * score[None]  # 0.14s
                    nms_box = torch.mm(weights, box) / weights.sum(
                        1, keepdim=True
                    )  # 0.55s

                box, label, score = (
                    nms_box / scale_factors[i],
                    nms_label,
                    score[keep],
                )  # 0.30s
                logits_cls = logits_cls[keep]
            results.append(
                dict(boxes=box, labels=label, scores=score, logits=logits_cls)
