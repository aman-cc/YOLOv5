...
def size_matched_idx(wh1, wh2, thresh):
    ...
    max_ratios = torch.max(ratios, 1.0 / ratios).max(2)[0]
...
def assign_targets_to_proposals(xy, size, overlap=0.5):
    ...
    offsets = xy.new_tensor(
        [[0, 0], [-overlap, 0], [0, -overlap], [overlap, 0], [0, overlap]]
    )
...
def batched_nms(
    boxes, scores, labels, threshold, max_size
...
def all_batched_nms(
    ids, boxes, scores, labels, threshold, max_size
):  # boxes format: (x1, y1, x2, y2)
