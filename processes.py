...
def train(args, resume_from, ckpt_file):
    ...
    dataset_train = yolo.datasets(
        yolo_args.dataset, file_roots[0], ann_files[0], train=True
    )
    dataset_test = yolo.datasets(
        yolo_args.dataset, file_roots[1], ann_files[1], train=True
    )  # set train=True for eval
    if len(dataset_train) < yolo_args.batch_size:
        raise Exception(
            f"Very low number of samples. Available samples: {len(dataset_train)} | Batch size: {yolo_args.batch_size}"
        )
    ...
    yolo_args.num_workers = min(
        os.cpu_count() // 2, 8, yolo_args.batch_size if yolo_args.batch_size > 1 else 0
    )
    ...
    if resume_from is not None:
        ...
        checkpoint = torch.load(
            ckpt_res_path, map_location=device
        ...
        if os.path.isfile(
            os.path.join(args["EXPT_DIR"], "yolov5s_official_2cf45318.pth")
        ):
            pre_trained_ckpt_path = os.path.join(
                args["EXPT_DIR"], "yolov5s_official_2cf45318.pth"
            )
            checkpoint = torch.load(
                pre_trained_ckpt_path, map_location=device
            )  # load last checkpoint
...
def infer(args, ckpt_file=None):
    ...
    ann_file = os.path.join(
        yolo_args.data_dir, "annotations", "instances_train2017.json"
    )
    if not os.path.isdir(args["EXPT_DIR"]):
        os.makedirs(args["EXPT_DIR"], exist_ok=True)
